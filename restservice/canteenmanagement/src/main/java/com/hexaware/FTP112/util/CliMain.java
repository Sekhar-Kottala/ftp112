 ...
 */
  private void showFullMenu() {
    Menu[] menu = MenuFactory.showMenu();
     System.out.println("-------------------------------------------------------------------------------------------------------------------------------------------");
    System.out.printf("%5s %20s %20s %20s %20s %20s %20s", "Menu Id", "Menu Name", "Menu Calories", "Vendor Id", "Ratings", "Reviews", "Price");
    System.out.println();
    System.out.println("--------------------------------------------------------------------------------------------------------------------------------------------");

    for (Menu menulist : menu) {
      System.out.format("%5s %20s %20s %20s %20s %20s %20s",
      menulist.getMenId(), menulist.getMenItem(), menulist.getMenCalories(), menulist.getVenId(), menulist.getRatings(), menulist.getReviews(), menulist.getMenPrice());
      System.out.println();
    }
  }
/**
 ...
 */
  private void showCustomerDetails() {
    Customer[] customerList = CustomerFactory.showCustomer();

    System.out.println("------------------------------------------------------------------------------");
    System.out.printf("%10s %20s %25s %20s", "Customer ID", "Customer Name", "Customer Phone Number", "Customer Username");
    System.out.println();
    System.out.println("-----------------------------------------------------------------------------");


    for (Customer customer : customerList) {
      System.out.format("%10d %20s %25s %20s",
                         customer.getCusId(), customer.getCusName(), customer.getCusPhoneNo(), customer.getCusUserName());
      System.out.println();                   
 ...
 */
  private void showVendorDetails() {
    Vendor[] vendorList = VendorFactory.showVendor();
    System.out.println("--------------------------------------------------------------------------------------------------");
 ...
 */
  private void showPendingOrders() {
    ...
    List<OrderItems> pendingList = OrdersFactory.showPendingOrders(vendorId);
     System.out.println("-----------------------------------------------------------------------------------------------------------------------------");
     System.out.printf("%10s %20s %20s %20s %20s %20s", "Order Item Id", "Menu Id", "Menu price", "Order Status", "Order quantity","Order id");
     System.out.println();
     System.out.println("-----------------------------------------------------------------------------------------------------------------------------");
     for (OrderItems orderItems : pendingList) {
      System.out.format("%10d %20d %20s %20s %20d %20d",
      orderItems.getOrderItemId(), orderItems.getMenId(), orderItems.getPrice(), orderItems.getStatus(), orderItems.getQuantity(), orderItems.getOrderId());
System.out.println();
     }
  ...
  private void showOrderHistory() {
    ...
    System.out.println("--------------------------------------------------------------------------------------------------");
     ...
     System.out.println();
      ...
      System.out.println("--------------------------------------------------------------------------------------------------");
    System.out.printf("%10s %10s %10s %20s %20s %20s %20s %15s", "Order Id",
              "Customer ID", "Vendor ID", "Order Date", "Total order Price",
              "Order Status", "Order Comments", "Wallet ID");
    System.out.println();
    System.out.println("--------------------------------------------------------------------------------------------------");
